...
Output files
FileDescriptionTFBS_all_sites.csvAll predicted binding sites with genomic coordinates and PWM scoresTF_summary.csvOne row per TF — site count, max/mean score, coordinate rangeTFBS_filtered_sites.csvSites matching your TF filter pattern (e.g. HIF-1, FOXO1)all_tfbs_plot.pdfSegment map of all TFs across the promoterfiltered_tfbs_plot.pdfSegment map for filtered TFs onlytop10_tfbs_plot.pdfPosition map + bar charts for top 10 TFs by PWM score

Requirements
R packages
rinstall.packages(c("ggplot2", "dplyr", "gridExtra"))
...
bashwget https://jaspar.elixir.no/download/data/2024/CORE/JASPAR2024.sqlite3
Set the path in config.R (see below).

Usage
1. Clone the repo
bashgit clone https://github.com/YOUR_USERNAME/TFBS-Mouse-mm39.git
cd TFBS-Mouse-mm39
2. Edit config.R
All user-facing parameters are in one place:
rENTREZ_ID      <- "208188"           # Entrez gene ID
...
3. Run
rsource("predict_tfbs.R")
Or from the command line:
bashRscript predict_tfbs.R

Parameters
ParameterDefaultNotesENTREZ_ID"208188"Mouse Entrez gene IDUPSTREAM2000Promoter window size (bp)MIN_SCORE"85%"Lower = more hits, more false positives. Raise to 90–95% for stricter resultsTF_FILTER"HIF-1|FOXO1|Nrf1"Regex pattern; case-insensitiveJASPAR_DB_PATH—Full path to local .sqlite3 fileOUTDIR"."Output directory
